...
rootdir = 'C:\\Users\\BioCraze\\Documents\\Ruthazer lab\\glial training\\data'
files =[f.path[:f.path.rfind('\\')+1] for i in glob.glob(f'{rootdir}/*/**/',recursive=True) for f in os.scandir(i) if f.path.endswith('.tif')]

for f in files:
    is_cell = np.load(f + 'max_proj\\plane0\\iscell.npy', allow_pickle=True)
    F = np.load(f + 'max_proj\\plane0\\F.npy',allow_pickle=True)
    ...
    np.savetxt(f + f[f.find("A"):-1] +'_processed_is_cell_traces.csv', F_fltrd, delimiter=", ", fmt="% 1.4f")
